...
The first test verifies that when a traveller first enrolls in the Frequent Flyer Member programme, then she will start out with Bronze status. This test is broken into two steps:

  . Create a new Frequent Flyer member
  . Check that the member has a status of Bronze

The second test verifies that a new Frequent Flyer member will earn Silver status after travelling 10,000 km. This test is also broken into steps:

In Serenity BDD, tests are broken down into reusable steps. An important principle behind Serenity BDD is the idea that it is easier to maintain a test that uses several layers of abstraction to hide the complexity behind different parts of a test. 

For example, the first test in the following sample is broken into two steps:
  
  Verify passenger can be enrolled into Frequent Flyer Member programme and by default s/he has Bronze membership status
...
----
<1> Steps classes can extend any class, or none. If it extends class it will be page object class
